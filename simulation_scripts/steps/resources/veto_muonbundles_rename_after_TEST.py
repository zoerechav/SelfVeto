...
class InjectVetoMuons(icetray.I3ConditionalModule):
        ...
        try:
            while True:
                ...
                if n_tries > MAX_TRIES:
                    raise RuntimeError(
                        f"Failed to sample bundle with ΣEμ ≤ Eν after {MAX_TRIES} tries"
                    )
    ...
    def _create_bundle_mc_tree(self, inj_energy,inj_pos, inj_time, inj_dir,depth,multiplicity):
        ...
        inj_muon_energies,energysample,bundle_diagnostics=self._sample_bundle_energies(multiplicity,inj_energy,np.cos(inj_dir.zenith),depth)
